...
class _BuildingInfoState extends State<BuildingInfo> with SingleTickerProviderStateMixin
{
	Widget build(BuildContext context)
	{
		...
		widget.selectedBuilding = (ModalRoute.of(context).settings.arguments as Map)["selectedGoods"];

		List<Widget> costs = List<Widget>();

		for (int i = 0; i < PopulationCalculator().goods[widget.selectedBuilding].length; i++) 
		{
			String info = PopulationCalculator().goods[widget.selectedBuilding].keys.elementAt(i);

			if (info == "costs")
			{
				PopulationCalculator().goods[widget.selectedBuilding][info].forEach((key, value)
				{
			...
			{
				costs.add(widget.globals.createChipWithValue("icons/other", "Balance", PopulationCalculator().goods[widget.selectedBuilding][info], context, widget.globals));
			}
		}

		return Scaffold(
				appBar: AppBar(
					title: Text('${PopulationCalculator().goods[widget.selectedBuilding]["building"]["name"]}'),
					elevation: 0,
					actions: <Widget>
					[
